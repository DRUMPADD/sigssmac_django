...
def modify_general(request):
    if request.method == 'POST':
        ...
        try:
            cursor = connections["mcgreen_db"].cursor()
            cursor.execute("MANTENIMIENTO_PREV_MODIFICAR", [responses.get("item_id"), responses.get("frec_"), responses.get("create_date"), responses.get("act_"), responses.get("date_next"), responses.get("prev_cod")])
...
def delete_general(request):
    if request.method == 'POST':
        ...
        try:
            cursor = connections["mcgreen_db"].cursor()
            cursor.callproc("MANTENIMIENTO_PREVENTIVO_ELIMINAR", [responses.get("prev_id")])
